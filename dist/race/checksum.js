...
export function computeRaceChecksum(pre) {
    const payload = {
        raceId: pre.id,
        ...
        firstTickPositions: pre.finalHorseStateMatrix?.[0]?.map((p) => p.position) ??
            pre.ticks[0]?.positions.map((p) => p.distance) ??
            [],
        lastTickPositions: pre.finalHorseStateMatrix?.[pre.finalHorseStateMatrix.length - 1]?.map((p) => p.position) ??
...
function sortFinishTimes(times) {
    const entries = Object.entries(times);
    entries.sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
    return Object.freeze(entries.map((e) => Object.freeze([e[0], e[1]])));
}
export function computeEventTimelineHash(tl) {

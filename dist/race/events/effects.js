...
function pickTwoDistinct(instance, count) {
    if (count <= 1)
        return [0, 0];
 ...
 */
export function applyEventEffects(baseHorsePaths, eventTimeline, eventCatalog) {
    ...
    for (const d of eventCatalog)
        defById.set(d.id, d);
    const stunUntil = new Map();
    const removed = new Map();
    const activeEventsWindows = new Map();
    const activeSwaps = new Map();
    const prevFinalPosition = new Map();
    for (const h of horseIds)
        prevFinalPosition.set(h, baseHorsePaths[0].find((x) => x.horseId === h)?.position ?? 0);
    const result = new Array(totalTicks);
    for (let tick = 0; tick < totalTicks; tick++) {
        const baseAtTick = baseHorsePaths[tick];
        const idxByHorse = new Map();
        ...
        const rerouteIfLucky = (targetIdx) => {
            if (targetIdx < 0 || targetIdx >= baseAtTick.length)
                return targetIdx;
        ...
        for (const ev of eventsHere) {
            ...
            if (ev.id === 'ufo_abduction') {
                ...
                markActive(horseId, ev.id, tick, Number.POSITIVE_INFINITY, activeEventsWindows);
                continue;
            }
            ...
            if (def.affectsMultipleHorses) {
                ...
                const horseId = baseAtTick[ix].horseId;
                if (removed.get(horseId))
                    continue;
                applyEffect(ev, def, horseId, tick, stunUntil, activeEventsWindows);
            }
        }
        const finalTickStates = new Array(baseAtTick.length);
        for (let i = 0; i < baseAtTick.length; i++) {
            ...
            const basePrev = tick > 0 ? baseHorsePaths[tick - 1][i].position : base.position;
            ...
            let offset = 0;
            if (isEventActive(horseId, 'hook_shot', tick, activeEventsWindows, true)) {
                offset -= OFFSETS.hookShotBackward;
            }
            if (isEventActive(horseId, 'rocket_boost', tick, activeEventsWindows, true)) {
            ...
            if (swap && tick >= swap.startTick && tick < swap.endTick) {
                ...
                if (otherIdx !== undefined) {
                    ...
                    let otherOffset = 0;
                    if (isEventActive(swap.withHorseId, 'hook_shot', tick, activeEventsWindows, true)) {
                        otherOffset -= OFFSETS.hookShotBackward;
                    }
                    if (isEventActive(swap.withHorseId, 'rocket_boost', tick, activeEventsWindows, true)) {
                        otherOffset += OFFSETS.rocketBoostForward;
                    }
                    candidatePos = Math.max(0, otherPrevFinal + otherMoveDelta + otherOffset);
    ...
    function applyEffect(ev, def, horseId, tick, stunUntilMap, active) {
        const endTick = tick + def.durationTicks;
        markActive(horseId, ev.id, tick, endTick, active);
        if (ev.id === 'bomb_throw') {
            stunUntilMap.set(horseId, Math.max(stunUntilMap.get(horseId) ?? tick, endTick));
        }
        // hook_shot and rocket_boost: instantaneous offsets at start tick handled in render
        // luck_charm: marker only
    }
    function markActive(horseId, id, startTick, endTick, active) {
        const arr = active.get(horseId) ?? [];
        arr.push({ id, startTick, endTick });
        active.set(horseId, arr);
    }
    function isEventActive(horseId, id, tick, active, onlyStartAtTick = false) {
        const arr = active.get(horseId);
        if (!arr)
            return false;
        if (onlyStartAtTick) {
        ...
        return arr.some((w) => w.id === id && tick >= w.startTick && tick < w.endTick);
    }
}

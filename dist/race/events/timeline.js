...
export const PACING_CURVE = Object.freeze({
    early: Object.freeze({
...
const DEFAULT_PHASE_PERCENTS = Object.freeze({
    earlyEndPct: 0.3,
    midEndPct: 0.7,
});
function clamp01(n) {
...
export function getRacePhases(raceDurationTicks, percents = DEFAULT_PHASE_PERCENTS) {
...
export function getRacePhaseForTick(tick, raceDurationTicks, percents = DEFAULT_PHASE_PERCENTS) {
    const phases = getRacePhases(raceDurationTicks, percents);
    const clamped = Math.max(0, Math.min(tick, raceDurationTicks - 1));
    const found = phases.find((p) => clamped >= p.startTick && clamped <= p.endTick);
    return (found?.id ?? 'final');
}
export function getPhaseWeightsForTick(tick, raceDurationTicks, pacingCurve = PACING_CURVE, percents = DEFAULT_PHASE_PERCENTS, rampMode = 'none') {
    const phases = getRacePhases(raceDurationTicks, percents);
    const clamped = Math.max(0, Math.min(tick, raceDurationTicks - 1));
    const idx = phases.findIndex((p) => clamped >= p.startTick && clamped <= p.endTick);
    if (idx < 0)
        return pacingCurve.final;
...
function maxWeight(weights) {
    return Math.max(weights.powerup, weights.combat, weights.environmental, weights.chaos);
}
function weightForCandidate(def, tickIndex, raceDurationTicks, options) {
    ...
    const weights = getPhaseWeightsForTick(tickIndex, raceDurationTicks, curve, perc, ramp);
...
export function getPerTickCategoryWeights(raceDurationTicks, options) {
    const curve = options?.pacingCurve ?? PACING_CURVE;
    const perc = options?.phasePercents ?? DEFAULT_PHASE_PERCENTS;
    const ramp = options?.rampMode ?? 'none';
    const out = [];
    for (let t = 0; t < raceDurationTicks; t++) {
        const weights = getPhaseWeightsForTick(t, raceDurationTicks, curve, perc, ramp);
        const phase = getRacePhaseForTick(t, raceDurationTicks, perc);
        out.push({ tick: t, phase, weights });
    }
    return Object.freeze(out.map((e) => Object.freeze({ ...e, weights: Object.freeze({ ...e.weights }) })));
}
/**
 ...
 */
export function generateEventTimeline(raceSeed, raceDurationTicks, catalog, options) {
    ...
    for (const def of catalog) {
        ...
        for (let occ = 0; occ < occMax; occ++) {
            const r = rng();
            const tickIndex = Math.min(raceDurationTicks - 1, Math.max(0, Math.floor(r * raceDurationTicks)));
    ...
    candidates.sort((a, b) => a.tickIndex - b.tickIndex || b.weight - a.weight || a.order - b.order);
    // 3) Place while enforcing constraints (spacing, concurrency, conflicts)
    const placed = new Map();
    const lastPlacedById = new Map();
    // Precompute a conflicts map for O(1) checks
    const conflictMap = new Map();
    for (const def of catalog)
        conflictMap.set(def.id, def.conflictsWith);
    for (const c of candidates) {
        ...
        const sameIdCount = atTick.reduce((acc, e) => (e.id === c.id ? acc + 1 : acc), 0);
        if (sameIdCount >= c.def.maxConcurrent)
            continue;
        // Conflicts at this tick (both directions)
        const conflict = atTick.some((e) => c.def.conflictsWith.includes(e.id) ||
            (conflictMap.get(e.id)?.includes(c.id) ?? false));
        if (conflict)
            continue;
        const instanceId = deterministicInstanceId(raceSeed, c.id, c.tickIndex, c.occ);
...
function deterministicInstanceId(seed, eventId, tickIndex, occ) {
    const key = `${seed}|${eventId}|${tickIndex}|${occ}`;
    const h = hashStringToInt(key) >>> 0;
    const hex = h.toString(16).padStart(8, '0');
    return `evt-${hex}`;
}
function getConflictsFor(catalog, id) {

...
function createBaseHorsePaths(totalTicks, horseCount) {
    return Object.freeze(Array.from({ length: totalTicks }, (_, tick) => Object.freeze(Array.from({ length: horseCount }, (_, index) => Object.freeze({
...
function createTimeline(eventId, instanceId) {
    return new Map([
        [
            0,
            Object.freeze([
...
function getAffectedHorseIds(matrix, tickIndex, eventId) {
    return matrix[tickIndex]
        .filter((horse) => horse.activeEvents.includes(eventId))
        .map((horse) => horse.horseId);
}
function findInstanceIdForAnchor(eventId, horseCount, targetAnchorIndex) {
    for (let attempt = 0; attempt < 10_000; attempt++) {
        const instanceId = `${eventId}-anchor-${targetAnchorIndex}-${attempt}`;
        const anchorIndex = hashStringToInt(`${instanceId}${eventId}-aoe`) % horseCount;
...
describe('applyEventEffects', () => {
    it('limits chain_reaction to four horses deterministically', () => {
        ...
        expect(stunnedAtStart.every((horse) => horse.activeEvents.includes('chain_reaction') &&
            horse.activeEvents.includes('chain_stun'))).toBe(true);
        expect(unaffectedAtStart.every((horse) => !horse.activeEvents.includes('chain_reaction') &&
            !horse.activeEvents.includes('chain_stun'))).toBe(true);
    ...
    it('applies tornado to the anchor and immediate neighbors only', () => {
        ...
        const result = applyEventEffects(baseHorsePaths, createTimeline('tornado', instanceId), EVENT_CATALOG);
    ...
    it('applies earthquake to a five-horse contiguous area when centered', () => {
        ...
        const result = applyEventEffects(baseHorsePaths, createTimeline('earthquake', instanceId), EVENT_CATALOG);
    ...
    it('limits bomb_throw to three horses', () => {
        ...
        const result = applyEventEffects(baseHorsePaths, createTimeline('bomb_throw', 'bomb-throw-cap-test'), EVENT_CATALOG);
        const affected = result[0].filter((horse) => horse.activeEvents.includes('bomb_throw'));
        expect(affected).toHaveLength(3);
        expect(affected.every((horse) => horse.isStunned)).toBe(true);
    });
    it('limits smg_attack to one horse', () => {
        const baseHorsePaths = createBaseHorsePaths(10, 10);
        const result = applyEventEffects(baseHorsePaths, createTimeline('smg_attack', 'smg-attack-cap-test'), EVENT_CATALOG);
        expect(getAffectedHorseIds(result, 0, 'smg_attack')).toHaveLength(1);
    });
    it('limits aerial_duel to two horses', () => {
        const baseHorsePaths = createBaseHorsePaths(10, 10);
        const result = applyEventEffects(baseHorsePaths, createTimeline('aerial_duel', 'aerial-duel-cap-test'), EVENT_CATALOG);
        expect(getAffectedHorseIds(result, 0, 'aerial_duel')).toHaveLength(2);
    });
    it('limits magnet_pull to one horse', () => {
        const baseHorsePaths = createBaseHorsePaths(10, 10);
        const result = applyEventEffects(baseHorsePaths, createTimeline('magnet_pull', 'magnet-pull-cap-test'), EVENT_CATALOG);
        expect(getAffectedHorseIds(result, 0, 'magnet_pull')).toHaveLength(1);
    });
    it('clamps instantaneous offsets at the finish line', () => {
        const baseHorsePaths = Object.freeze([
            Object.freeze([
                Object.freeze({
                    horseId: 'horse-1',
                    position: 95,
        ...
        const result = applyEventEffects(baseHorsePaths, createTimeline('rocket_boost', 'finish-line-clamp'), EVENT_CATALOG);

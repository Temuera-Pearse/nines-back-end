...
describeIfDatabase('bet settlement integration', () => {
    ...
    it('settles winning and losing bets idempotently', async () => {
        ...
        expect(firstSettlement.settledBets.find((entry) => entry.bet.resultStatus === 'won')?.ledgerEntry
            ?.entryType).toBe('settlement_credit');
        ...
        const pool = getPool();
        const settlementLedgerCount = await pool.query(`
      ...
      `, [raceId]);
        ...
        const winnerWalletAfterRerun = await walletService.getWalletByUserId(winner.user.id, 'USD');

...
async function applyMigrations() {
    const migrationPath = fileURLToPath(new URL('../../db/migrations/001_alpha_race_metadata.sql', import.meta.url));
...
describeIfDb('raceReadService integration', () => {
    ...
    it('reads current and archived races from Postgres and loads artifact fallbacks from disk', async () => {
        ...
        await fs.writeFile(finalTicksPath, JSON.stringify([
            [{ position: 10 }, { position: 8 }],
            [{ position: 25 }, { position: 20 }],
        ]), 'utf8');
        await fs.writeFile(rawTicksPath, JSON.stringify([

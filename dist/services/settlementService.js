...
function calculatePayoutMinor(bet, winnerId) {
    if (bet.selectionId !== winnerId)
        return 0n;
    return bet.stakeMinor * 2n;
}
export class DefaultSettlementService {
    betRepository = getBetRepository();
    raceRepository = getRaceRepository();
    async settleRaceBets(raceId) {
        ...
        try {
            ...
            const unsettledBets = await this.betRepository.listUnsettledBetsByRaceId(raceId, client);
            const settledBets = [];
            let wonCount = 0;
            let lostCount = 0;
...
let sharedSettlementService = null;

...
describeIfDatabase('user and wallet integration', () => {
    ...
    beforeAll(async () => {
        ...
        initPool();
        const pool = getPool();
        const raceMigration = readFileSync(resolve('db/migrations/001_alpha_race_metadata.sql'), 'utf8');
        const userMigration = readFileSync(resolve('db/migrations/002_alpha_users_wallets.sql'), 'utf8');
        await pool.query(raceMigration);
        await pool.query(userMigration);
    });
    beforeEach(async () => {
        const pool = getPool();
        await pool.query('truncate table wallet_ledger_entries restart identity cascade');

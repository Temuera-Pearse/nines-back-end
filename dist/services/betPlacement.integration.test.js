...
describeIfDatabase('bet placement integration', () => {
    ...
    beforeEach(async () => {
        const pool = getPool();
        await pool.query('truncate table bets, wallet_ledger_entries, wallets, users, race_artifacts, races restart identity cascade');
    ...
    it('places a bet atomically and debits the wallet', async () => {
        ...
        const pool = getPool();
        const betCount = await pool.query(`select count(*)::int as count from bets where user_id = $1`, [created.user.id]);
        const ledgerCount = await pool.query(`select count(*)::int as count from wallet_ledger_entries where wallet_id = $1`, [created.wallet.id]);
    ...
    it('rejects invalid selections without debiting the wallet', async () => {
        ...
        await expect(betService.placeBet({
            userId: created.user.id,
            raceId,

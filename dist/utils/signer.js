...
let privateKeyPem = process.env.TICK_SIGNING_PRIVATE_KEY || null;
let publicKeyPem = process.env.TICK_SIGNING_PUBLIC_KEY || null;
let keyId = null;
const ssm = new SSMClient({ region: process.env.AWS_REGION || 'us-east-1' });
const secrets = new SecretsManagerClient({
    region: process.env.AWS_REGION || 'us-east-1',
});
async function loadKeysFromAws() {
    try {
        ...
        if (secId) {
            const out = await secrets.send(new GetSecretValueCommand({ SecretId: secId }));
            const val = out.SecretString ||
                (out.SecretBinary
...
export async function getPublicKeyIdAsync() {
    await ensureKeysAsync();
    return keyId;
}
